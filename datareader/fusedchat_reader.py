...
from datareader import data_config
    ...
    def define_instruction(self,child_dialogue):
        ...
        if 'dialog_act' in child_dialogue[-1].keys():
            dict_input['prompt'] = instruction.replace("<DIALOGUE_CONTEXT>",
                                                       ''.join([turn for turn in list_turn])).replace('<DOMAIN>',
                                                                                                     "")
        else:
            ...
            domain = set()
            for i in service:
                domain.add(list(i.split("-"))[0])

            for i in domain:
                for y in ontology.__dir__():
                    if i in y:
                        list_domain.append(getattr(ontology, y))
                dict_input['prompt'] = instruction.replace("<DIALOGUE_CONTEXT>",
                                                       ''.join([turn for turn in list_turn])).replace('<DOMAIN>', " AND ".join(list_domain))
